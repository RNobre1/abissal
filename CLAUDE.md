...
> - **Teste de módulo puro NÃO prova fiação.** Para cada coisa que produz um número, exista UM teste que atravesse do produtor ao valor final. Dois pipelines rodaram meses produzindo números que ninguém consumia — o `SeasonAvgs` não chegava em λ e a `league` nunca existiu no `detail_json` (B58). Ao consertar o produtor, varra TODOS os chamadores no mesmo passe.
> - **O PostgREST corta em 1.000 linhas**, qualquer que seja o `.limit()`. Pagine com `.range()` via `lib/supabase/paginated-fetch.ts#fetchAllPages`. Dublê de teste tem que ser tão mesquinho quanto a produção (B59).
> - **Ausência de dado é um ESTADO, nunca um número plausível.** Prior genérico, `0` de produtor externo e resposta truncada têm a mesma assinatura: não falham, devolvem algo que parece certo (B50/B52/B57/B59).
